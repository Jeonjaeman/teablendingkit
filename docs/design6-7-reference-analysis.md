...
- **전체메뉴(GNB) 세로 반응형**(2026-09-17): 1920×940 등 낮은 화면에서 5개 항목이 넘쳐 스크롤·겹침 → 항목 패딩·타이틀·서브 크기를 `vh` 기반 clamp로, 패널 미장은 `background-attachment:local` 레이어로, 패널 자체 닫기 버튼 제거(헤더 X만), 열렸을 때 헤더 CTA는 페이드아웃. 확인: `docs/preview-d7-gnb-2026-09-17.jpg`.
...
- [ ] design7 슬라이더 자동재생 접근성(pause on hover) — 언어 전환(KR/EN/FR)은 2026-09-17 대표 요청으로 제거
